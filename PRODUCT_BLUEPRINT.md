...
**Not built, and why:** true first-run/brand-new-user detection is effectively unreachable — every provider seeds present-state demo data on first hydration (an established, disclosed pattern, not new here), so a genuinely empty Home never occurs in practice today. `HomeHeader`'s "add your first task" signal is real code, reachable only if a user manually clears every task, deliverable, and habit back to zero — a legitimate "back to nothing" state, just not "brand new" in the literal sense. Building real first-run detection needs an explicit "start fresh vs. explore the sample" choice (§28's gap #4, its own recommended next phase), not something this Home-only pass should invent as a side effect.
...
- **Home:** ✅ **Resolved (Home v2 redesign, §9.6).** §9.2's dominant-answered-question vision — orientation, a real plan, real workload risk, real schedule — is now built, not a placeholder. Two accessibility items flagged since Phase 2 (§22 items 3, 7) were **not** addressed this pass (out of this phase's explicit IA/hierarchy scope) and remain genuinely open.
...
2. **Home becomes the decision surface** — surface Plan's existing workload-risk engine directly on Home (no new computation, just visibility) + fix the two long-open accessibility items. This is the "aha" moment the roadmap has pointed at since §4, only honestly buildable now that Phase 1 makes the numbers real. ✅ **Shipped as the Home v2 redesign (§9.6)** — the decision-surface piece (Attention, real Schedule, guidance-framed free time) landed as a full information-architecture replacement, not an addition to the old cards. The two accessibility items (§22 #3, #7) were **not** in scope for this pass and remain open.
